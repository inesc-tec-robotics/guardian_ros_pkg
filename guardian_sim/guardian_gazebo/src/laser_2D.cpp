...
namespace gazebo
{
    ...
    void GazeboRosLaser::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
    {
      ...
      if (!this->sdf->HasElement("time_increment"))
      {
        ROS_INFO("Laser plugin missing <time_increment>, defaults to 0");
        this->time_increment_ = 0;
      }
      else
      {
        this->time_increment_ = this->sdf->Get<float>("time_increment");
      }


      if (!this->sdf->HasElement("intensities"))
      {
        ROS_INFO("Laser plugin missing <intensities>, defaults to false");
        this->intensities = false;
      }
      else
      {
        this->intensities = this->sdf->Get<bool>("intensities");
      }
    ...
    void GazeboRosLaser::LoadThread()
    {
      ...
      if (this->topic_name_ != "")
      {
        ...
        ros::AdvertiseOptions::create<sensor_msgs::LaserScan>(this->topic_name_, 1,boost::bind(&GazeboRosLaser::LaserConnect, this),
...
void GazeboRosLaser::OnScan(ConstLaserScanStampedPtr &_msg)
{
  ...
  laser_msg.time_increment = this->time_increment_; // time between measurements // 0 = instantaneous simulator scan

  this->current_time = laser_msg.header.stamp;
  this->scan_time = (this->current_time - this->previous_time);

  laser_msg.scan_time = this->scan_time.toSec(); // time between two scan messages
  ...
  previous_time = laser_msg.header.stamp;
