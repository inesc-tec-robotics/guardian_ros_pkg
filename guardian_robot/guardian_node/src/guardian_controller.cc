...
int guardian_controller::ReadEncoders(){
    ...
    if(status != RQ_SUCCESS){
        ...
        this->robot_data.encoder_left = GUARDIAN_CONTROLLER_ENCODER_DIR * enc;
        bEncL = true;
    }
    //Wait 10 ms before sending another command to device
	sleepms(10);
    ...
    if(status != RQ_SUCCESS){
        ...
        this->robot_data.encoder_right = GUARDIAN_CONTROLLER_ENCODER_DIR * enc;
        bEncR = true;
    }
    //Wait 10 ms before sending another command to device
	sleepms(10);
...
void guardian_controller::UpdateOdometry(){
   ...
   // ROS_INFO("guardian_controller::CalcRPM OK. v_left_mps = %lf,  v_right_mps = %lf",v_left_mps,v_right_mps);
	...
	//ROS_INFO("guardian_controller::CalcRPM OK 1 pa = %lf", robot_pose.pa);
	radnorm(&robot_pose.pa);
	//ROS_INFO("guardian_controller::CalcRPM OK 2");
	...
	robot_data.rpm_right = (float)fVelocityLeftRpm;   // Motor RPM
	robot_data.rpm_left  = (float)fVelocityRightRpm;
	//ROS_INFO("guardian_controller::CalcRPM OK 3");

	//ROS_INFO("guardian_node::UpdateOdometry");
